...
KernelBasedTracker::KernelBasedTracker(int bin_num,const Mat &frame,Rect &box, int image_w, int image_h)
:bin_num_(bin_num)
{
...
KernelBasedTracker::KernelBasedTracker(KernelBasedTracker& tracker)
{
    bin_num_ = tracker.bin_num_;
    tracker.kernel_.copyTo(kernel_);
    box_ = tracker.box_;
    tracker.target_model_.copyTo(target_model_);
}
...
void KernelBasedTracker::CalcMeanShift(const Mat &frame, bool multi_scale_box_flag)
{
    ...
    for(;box_iter!=box_list.end();box_iter++)
    {
        ...
        while (iter <= 20)
        {
            ...
            //adjust the size of box
            AdjustBox(candidate_box);
...
void KernelBasedTracker::AdjustBox(Rect &box)
{
    if (box.x < 0)
        box.x = 0;
    if (box.y < 0)
    ...
    if (box.x + box.width > MAXW - 1) {
        box.width = MAXW - 1 - box.x;
    }
    if (box.height > MAXH - 1) {
        box.height = MAXH - 1 - box.y;
    }
}
